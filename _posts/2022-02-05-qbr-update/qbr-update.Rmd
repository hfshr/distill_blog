...
I initially developed `qbr` using the `htmlwidgets` framework, guided by an earlier attempt at integrating the library with R by [\@harveyl888](https://github.com/harveyl888/queryBuilder). However after using `qbr` in a couple of projects at work, I found there were a few limitations, particularly when it came to updating the widget with new filters. So this latest attempt moves away from the `htmlwidget` approach and instead uses a custom input binding for Shiny, and implements some additional functionality, all with the aim of making the library easier to use and customise as required. 
