...
The package is really simple. It has two functions, the first gets the data for a individual athlete, while the second can be used to get the rankings for a given year, event and gender.
...
rankings %>% 
  ...
  ggplot(., aes(x = year, y = perf, 
              ...
              label = label))+
  geom_point() +
  geom_segment(aes(x=year, y=perf, xend = year, yend =min(perf)-min(perf)*0.01))+
...
So that's a quick tour of `poweRof10`. Not much too it, but I've had some fun playing with this data so thought I'd share it anyway. 
