...
description: |
  This post explores the use of Bayesian networks using the excellent {bnlearn} package to examine the relationship between different risk factors and the probability of sustaining a sports injury.
author:
  - name: Harry Fisher
...
In this post I want to share a few details about the humble Bayesian Network (BN). While not generally considered one of the "mainstream" forms of analysis, Bayesian networks have several features that make them an interesting choice, including their flexibility, interpretability and visually appealing nature. I used BNs during PhD, and was fortunate enough to receive some guidance from [Marco Scutari](https://www.bnlearn.com/about/), the author of `bnlearn` [@R-bnlearn_a] and who is a leading expert in the field when it comes to all things BN. From the outset, I want to clarify that I am by no means an expert in Bayesian networks (far from it!), and encourage any reader who wants to learn more about the theory underpinning BNs, or to see more in depth examples, to visit Marco's site which is a fantastic place to start. I have also included a list of resources I found helpful when learning about BNs at the end of the post. That said, in this post I hope to show a motivating example that might spark some ideas about how BNs can be used!
...
The data I've used here comes from my PhD, where I was investigating the relationships between stress-related factors and the risk of sports injury.

<aside>

I have simplified the data slightly here just to make the examples clearer. If interested in the full study, you can find the paper here:
...
black_list <- expand.grid(from = explanatory_vars,
                          to = control_vars) %>% 
  bind_rows(., 
            data.frame(from = c("clevel", "nlebase", "nlebase"),
...
You can drag the nodes around and hover over them for more information regarding the arcs.
...
So that was a very quick intro to Bayesian Networks! As promised, below I have included some resources that I'd highly recommend if you what a more. Thanks for reading!
