...
mod_tuned <- update(mod, select_best(tuned, "pr_auc"))
...
```{r eval=FALSE}
final_res <- fit(xgboost_wkflow_tuned, train)
```
...
preds <- tibble(truth = test$Status) %>% 
  bind_cols(., predict(final_res, new_data = test[,-1])) %>% 
  bind_cols(., predict(final_res, new_data = test[,-1], type = "prob"))
...
```{r}
conf_mat(preds, truth, .pred_class)
...
```{r}
summary(conf_mat(preds, truth, .pred_class)) %>%
  select(-.estimator) %>%
