...
While maximising a models performance is often desirable, it can sometimes limit the _explainability_. Being able to understand why your model is making certain predictions is important, particularly if you are using a model to make important business decision. This post is going to explore how we can use [SHapley Additive exPlanations](https://github.com/slundberg/shap) to dig a little deeper into complex models in an attempt to understand _why_ certain predictions are made.
...
First, we need to supply the `fastshap::explain()` function with the features we used to train the model. As we used some preprocessing steps, we'll need to prep and juice our training data to ensure it is the same as the data that was used in the model.
...
So that was a quick look at the excellent `fastshap` package and what it has to offer. As models become increasingly complex, the tools to help explain them become even more important and SHAP provides a great way to shine a light into the "black box" of the inner workings of complex models.
