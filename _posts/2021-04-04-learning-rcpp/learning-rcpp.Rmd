...
Few extra things have happened and I'll attempt to explain... First we create `result` which is a numeric vector the size of the vector that we pass in to the function. Then we use a for loop with the syntax `for(init; check; increment)`. We initiate the loop with a new variable `i = 0` and keep looping until i is less than n (the size of our vector). Finally the `++i` tells the loop to increment the value of i each time the loop completes.
...
Pretty cool despite this function being entirely useless as you could easily do the same thing in R.

Another cool feature is the ability to use R functions in c++ code. Here is a another completely useless example using the `emo::ji` function with a string input.
